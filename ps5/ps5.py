...
def filter(pixels_list, color):
    ...
    """
    filter = make_matrix(color)
    filtered_pixel_lists = []
    for pixel in pixels_list:
        filtered_pixel = tuple(int(i) for i in matrix_multiply(filter, pixel))
        filtered_pixel_lists.append(filtered_pixel)
    return filtered_pixel_lists
...
def reveal_color_image(filename):
    ...
    pixels_list = []
    for i in im:
        pixels_list.append(tuple(int(j * 255 / 7) for j in extract_end_bits(3, i)))
    with Image.open(filename) as im:
        revealed_im = pix_to_img(pixels_list, im.size, im.mode)
    return revealed_im
