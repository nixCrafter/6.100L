...
def output_to_file(
    test_case_name, word_to_guess, guessed_letters, student_output, correct_output
):
...
class TestPS2(unittest.TestCase):
    ...
    def test_has_player_won_repeated_letters(self):
        self.assertTrue(
            student.has_player_won("bass", ["a", "s", "b", "e"]),
            "Failed with repeated letters",
        )
        self.assertFalse(
            student.has_player_won("rare", ["f", "t", "r", "e"]),
            "Failed with repeated letters",
        )

    def test_has_player_won_empty_string(self):
        self.assertTrue(
            student.has_player_won("", ["f", "c", "y", "e"]),
            "Failed with the empty string",
        )

    def test_has_player_won_empty_list(self):
        self.assertFalse(
            student.has_player_won("code", []), "Failed with the empty list"
        )

    def test_get_word_progress(self):
        self.assertTrue(
            compare_results(
                student.get_word_progress("face", ["f", "c", "a", "e"]), "face"
            )
        )
        self.assertTrue(
            compare_results(
                student.get_word_progress("moves", ["o", "c", "a", "v", "e"]),
                HIDDEN + "ove" + HIDDEN,
            )
        )
    ...
    def test_get_word_progress_repeated_letters(self):
        self.assertTrue(
            compare_results(
                student.get_word_progress("bass", ["a", "s", "b", "e"]), "bass"
            ),
            "Failed with repeated letters",
        )
        self.assertTrue(
            compare_results(
                student.get_word_progress("rare", ["f", "t", "r", "e"]),
                "r" + HIDDEN + "re",
            ),
            "Failed with repeated letters",
        )
    ...
    def test_get_word_progress_empty_string(self):
        self.assertTrue(
            compare_results(student.get_word_progress("", ["f", "c", "y", "e"]), ""),
            "Failed with the empty string",
        )

    def test_get_word_progress_empty_list(self):
        self.assertTrue(
            compare_results(student.get_word_progress("code", []), HIDDEN * 4),
            "Failed with the empty list",
        )

    def test_get_available_letters(self):
        self.assertEqual(
            student.get_available_letters(["a", "b", "c", "d"]),
...
point_values = {
    "test_has_player_won": 0.40,
    "test_has_player_won_repeated_letters": 0.40,
    "test_has_player_won_empty_string": 0.40,
    "test_has_player_won_empty_list": 0.40,
    "test_get_word_progress": 0.40,
    "test_get_word_progress_repeated_letters": 0.40,
    "test_get_word_progress_empty_string": 0.40,
    "test_get_word_progress_empty_list": 0.40,
    "test_get_available_letters": 0.40,
    "test_get_available_letters_empty_string": 0.40,
    "test_get_available_letters_empty_list": 0.40,
    "test_play_game_short": 0.35,
    "test_play_game_short_fail": 0.35,
    "test_play_game_wildcard": 0.30,
}
